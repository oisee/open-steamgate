...
describe("the seed inserts every row it has, in batches", () => {
  ...
  it("names every table that has a data file, and no other", () => {
    const inFiles = new Set(readdirSync("data").filter((f) => f.endsWith(".tabu.json"))
  ...
  it("carries as many rows as the files hold", () => {
    ...
    for (const file of readdirSync("data").filter((f) => f.endsWith(".tabu.json"))) {
      const table = file.slice(0, -".tabu.json".length).toLowerCase();
      const count = JSON.parse(readFileSync(join("data", file), "utf8")).length;
      if (count === 0) continue;
  ...
  it("is far fewer statements than rows, which is the point", () => {
    const rows = readdirSync("data").filter((f) => f.endsWith(".tabu.json"))
      .reduce((n, f) => n + JSON.parse(readFileSync(join("data", f), "utf8")).length, 0);
