...
export const COLUMN_RENAMES = [
  ...
  // activate there (ANORMALIES zone-reserved-word). The CDS element keeps the
  // alias Zone, so the OData property is still ZONE.
 ...
 *  Returns the renames applied, as "table.from -> to". */
export async function migrateDuckdbColumns({query, execute}, renames = COLUMN_RENAMES) {
  ...
  for (const {table, from, to} of renames) {
    ...
    if (have === undefined || !have.has(from)) continue;
    if (have.has(to)) {
      throw new Error(`${table} has both ${from} and ${to}; an old column next to the new one is not migrated automatically`);
    }
 ...
 *  from `statements` (the DuckDB schema, in its own order). Returns their
 *  number. */
export async function refreshDuckdbViews({execute}, statements) {
  const views = [statements].flat().map(String)
