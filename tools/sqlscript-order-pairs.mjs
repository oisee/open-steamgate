...
export const ORDERED = [
  ...
  {id: "ties through a renamed key", frozen: order(project(IT_ROWS, [{as: "KK", expr: k}, {as: "N", expr: n}]), [{col: "KK", desc: false}])},
...
export const PROCEDURE = [
  ...
  {id: "ties: ORDER BY K, the loop reads N", body: LOOP("SELECT n, k FROM :it ORDER BY k")},
...
export async function pairs() {
  const out = [{kind: "header", note: "the order a cursor's rows come in, as tools/sqlscript-to-procedure-ir.mjs (orderOf) and tools/sqlscript-procedure-ir.mjs (orderedRelation) decide it; a port gives the same answers, SQL bytes and rows",
    conventions: [
      ...
      "only the cursor query's own ORDER BY is defined (guaranteed); the caller's rows are defined (observed); scan, filter without a subquery, projection without a window or subquery inherit; everything else is unknown",
  ...
  try {
    for (const one of ORDERED) {
      ...
      for (const [name, dialect, client] of list) {
        ...
        const seen = rows.map((row) => Object.fromEntries(Object.entries(row).filter(([c]) => !c.startsWith("__ORD")).map(([c, v]) => [c.toUpperCase(), cell(v)])));
