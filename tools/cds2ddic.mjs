...
function publishedYaml(e) {
  ...
  lines.push(`    creatable: false`);
  lines.push(`    updatable: false`);
  lines.push(`    deletable: false`);
  lines.push(`    operations: [R, Q]`);
  ...
  for (const f of e.fields) {
    ...
    lines.push(`      ${propertyName(f.name)}: {type: ${type}, field: ${f.name}, readonly: true, label: ${label}${f.virtual ? ", sortable: false, filterable: false" : ""}}`);
  }
  return lines.join("\n") + "\n";
}
