...
CLASS ltcl IMPLEMENTATION.
  ...
  ENDMETHOD.
  METHOD f1_refused.
    out( zcl_osg_httpc_probe=>call( iv_url = `http://localhost:1/x` ) ).
  ENDMETHOD.
  METHOD f2_tls_to_plain.
    out( zcl_osg_httpc_probe=>call( iv_url = `https://localhost:50000/sap/bc/abap/demo_post` ) ).
  ENDMETHOD.
  METHOD f3_bad_scheme.
    out( zcl_osg_httpc_probe=>call( iv_url = `ftp://localhost:50000/x` ) ).
  ENDMETHOD.
  METHOD f4_header_newline.
    out( zcl_osg_httpc_probe=>call( iv_url = zcl_osg_httpc_probe=>base && `/sap/bc/abap/demo_post`
  ...
  METHOD f7_receive_before_send.
    ...
    cl_http_client=>create_by_url( EXPORTING url = zcl_osg_httpc_probe=>base && `/sap/bc/abap/demo_post` IMPORTING client = li_client ).
  ...
  ENDMETHOD.
  METHOD s1_403.
    out( zcl_osg_httpc_probe=>call( iv_url = zcl_osg_httpc_probe=>base && `/sap/public/ping` iv_ticket = abap_false ) ).
  ENDMETHOD.
  METHOD s2_404.
    out( zcl_osg_httpc_probe=>call( iv_url = zcl_osg_httpc_probe=>base && `/sap/bc/abap/zz_osg_no_such_node` ) ).
  ENDMETHOD.
  METHOD s3_401.
    out( zcl_osg_httpc_probe=>call( iv_url = zcl_osg_httpc_probe=>base && `/sap/bc/abap/demo_post` iv_ticket = abap_false ) ).
  ENDMETHOD.
  METHOD s4_500.
  ...
  METHOD q2_post_query_nobody.
    out( zcl_osg_httpc_probe=>call( iv_url = zcl_osg_httpc_probe=>base && `/sap/bc/abap/demo_post?input=X&k=v`
