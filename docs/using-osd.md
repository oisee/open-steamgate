...
```
npm run stg:compile -- src/my/zmy_srv.stg.yaml --out gen/stg
```
...
The service is then `Z<VIEW>_CDS` and is served like any other.
...
ABAP frames under it, because the transpile carries source maps. `npm run dev`
